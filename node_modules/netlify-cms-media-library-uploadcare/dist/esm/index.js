...
function isFileGroup(files) {
  const basePatternString = "~".concat(files.length, "/nth/");

  const mapExpression = (val, idx) => new RegExp("".concat(basePatternString).concat(idx, "/$"));
...
function getFileGroup(files) {
   ...
   */
  const groupId = new RegExp("^.+/([^/]+~".concat(files.length, ")/nth/")).exec(files[0])[1];
  /**
...
function openDialog(_ref) {
  ...
  const buildUrl = fileInfo => {
    ...
    let url = isImage && settings.defaultOperations ? "".concat(cdnUrl, "-").concat(settings.defaultOperations) : cdnUrl;
  ...
  _uploadcareWidget.default.openDialog(files, config).done((_ref2) => {
    let {
      promise,
      files
    } = _ref2;
