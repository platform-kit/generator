      ...
      <div class="container px-3">
        ...
        >
          ...
          <div
            ...
            class="col-12 col-md-3 px-0 pr-md-2 mb-4 d-inline-block d-xl-flex"
          >
            <div
              class="card raised border-0 mb-1 pull-left mx-0 px-0 h-100 mr-md-3 px-0 pb-3 m-0 justify-content-center w-100"
              :key="edge.node.id"
              :post="edge.node"
...
<page-query>
query {
  products: allOffering(sort: [{ by: "price" }, { by: "boost" }] order: DESC, filter: { featured: { eq: true }, type: { eq: "subscription"} }) {
    edges {
      node {
        id
        title
...
export default {
  ...
  async mounted() {
    try {
      this.productsPage = await this.$fetch('/pricing');
    } catch (error) {
      console.log(error);
    }
  },
  methods: {
    collectionHasProducts(collection){ 
      ...
      var results = allProducts.filter(product => product.node.type == 'subscription' && collection.node.offerings.includes(product.node.id));
