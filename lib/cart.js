    ...
    async push(items) {
        ...
        try {
            ...
            if(plan != null) {
                ...
                if(items[0].plan.attributes.trial_days != null && typeof items[0].plan.attributes.trial_days == 'number'){
                    object.trial_days = items[0].plan.attributes.trial_days;
                }
                try {
                stripe.plans.update(
                    items[0].plan.attributes.id + '-' + items[0].plan.attributes.price + '-' + items[0].plan.attributes.interval,
                    object,
                    function(err, plan) {
